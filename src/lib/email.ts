...
export async function sendPasswordResetEmail(params: PasswordResetParams): Promise<void> {
  ...
  if (process.env.NODE_ENV === "development" && process.env.DEV_SKIP_EMAIL === "true") {
    console.log("[Email DEV] Password reset for:", params.to);
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;margin:40px auto;background:#ffffff;border-radius:8px;padding:32px;">
    <tr>
      <td>
        ...
        <p style="margin:0 0 24px;">
          <a href="${resetUrl}" style="display:inline-block;background:#2C5F7C;color:#ffffff;text-decoration:none;padding:12px 24px;border-radius:6px;font-size:14px;font-weight:bold;">
            Reset Password
          </a>
        </p>
        <p style="font-size:12px;color:#888;line-height:1.5;margin:0 0 24px;">
